...
# 标准化
def z_score(x):
    '''x∗=(x−μ)/σ μ是均值，σ是标准差, 标准差 s = sqrt( ((x1-x)² + (x2-x)² +...+ (xn-x)²) / n )  '''
    x_mean = np.mean(x)
    s2 = np.sqrt(sum([(i - np.mean(x)) * (i - np.mean(x)) for i in x]) / len(x))
