...
def AlexNet(input_shape=(224,224,3),output_shape=2):
    # AlexNet
    model = Sequential()
    # 使用步长为4x4，大小为11的卷积核对图像进行卷积，输出的特征层为96层，输出的shape为(55,55,96)； (size - keral + 2p) / step
    # 所建模型后输出为48特征层，自己定义，即使用了48个不同的卷积核
    model.add(
        Conv2D(
            filters=48, # 输出特征
            kernel_size=(11,11),
            strides=(4,4),
            padding='valid', # 不使用任何填充
            input_shape=input_shape,
            activation='relu'
        )
    )
    
    model.add(BatchNormalization()) # 批次标准化层,有助于加速收敛过程并提高模型的泛化能力
