...
import tf_slim as slim   #提供搭建神经网络模型时许多函数的标准实现
# 创建slim对象
...
def vgg_16(inputs,
    ...
    with tf.variable_scope(scope,'vgg_16',[inputs]):
        '''
               tf.variable_scope：一个上下文管理器（context manager），它用于为变量和参数化变量的创建提供范围（scope）。它有助于避免变量命名冲突，并可为模型的不同部分提供一个组织框架。
               scope：代表当前的变量范围。
               'vgg_16'：指定了将要创建的变量范围的名称。
               '''
        # 建立vgg_16的网络

        # conv1两次[3,3]卷积网络，输出的特征层为64，输出为(224,224,64)
        net = slim.repeat(inputs, 2, slim.conv2d,64,[3,3],scope='conv1')
        # 2X2最大池化，输出net为(112,112,64)  默认：stride=2,padding=valid
        ...
        if spatial_squeeze:
            net = tf.squeeze(net, [1, 2], name='fc8/squeezed')  # 将 (1, 1, num_classes) 的张量变扁平化为 (num_classes) 的张量
            # squeeze（net）删除所有为1的维度， 加了[1,2]删除1 2 位为1的，也就是h w .就将卷积拍平了
        return net
