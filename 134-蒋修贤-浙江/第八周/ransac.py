	...
	while iterations < k:
		...
		if(len(also_inliers)>0):
			...
			if thiserr < besterr:
				bestfit = bettermodel
				besterr = thiserr
				best_inlier_idx = np.concatenate((maybe_idxs,also_idxs))
		iterations += 1
	if bestfit is None:
		return ValueError("didn`t meet fit acceptance criteria")
	if return_all:
...
def test():
	...
	debug = False
	mode = LinearLeastSquareModel(input_columns,output_columns,debug = debug)
	
	linear_fit,reside,rank,s = sp.linalg.lstsq(all_data[:,input_columns],all_data[:,output_columns])
	
	ransac_fit, ransac_data = ransac(all_data, mode, 50, 1000, 7e3, 300, debug = debug, return_all=True)
	if 1:
		import pylab
