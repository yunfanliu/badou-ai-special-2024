...
def read_cifar10(file_queue):
    result = CIFAR10Record()  # 相当于用于记录的

    label_bytes = 1  # cifar10的一个参数，1代表我这块用的是cifar 10 而不是cifar  100 ，如果是Cifar-100数据集，则此处为2
    result.height = 32
    result.width = 32
    ...
    depth_major = tf.reshape(tf.strided_slice(record_bytes, [label_bytes], [label_bytes + image_bytes]),
                             # reshape是维度的数量已经变化啦，transpose改变的是维度的顺序
...
def inputs(data_dir, batch_size, distorted):  # 这个函数就对数据进行预处理---对图像数据是否进行图像增强进行判断，并作出相应的操作
    ...
    if distorted != None:  # 如果预处理函数中的distorted参数不为空值，就代表要进行图片增强处理，并没有改变他的特征的
