...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    iterations = 0
    bestfit = None
    besterr = np.inf  # 设置最大默认值
    best_inlier_idxs = None
    while iterations < k:
        maybe_idxs, test_idxs = random_partition(n, data.shape[0])  # 选择内外索引
        maybe_inliers = data[maybe_idxs, :]  # 获取内群点
        test_points = data[test_idxs]  # 获取其他点
        maybemodel = model.fit(maybe_inliers)  # 用内群点拟合模型
        test_err = model.get_error(test_points, maybemodel)  # 计算误差:平方和最小
        also_idxs = test_idxs[test_err < t]  # 判断其他点是否符合内群点条件
        also_inliers = data[also_idxs, :]
        if len(also_inliers) > d:  # 判断内群点数是否符合要求样本数
            betterdata = np.concatenate((maybe_inliers, also_inliers))  # 样本合并
            ...
            if thiserr < besterr:
                bestfit = bettermodel
                besterr = thiserr
                best_inlier_idxs = np.concatenate((maybe_idxs, also_idxs))  # 更新内群点,将新点加入
        iterations += 1
    if bestfit is None:
