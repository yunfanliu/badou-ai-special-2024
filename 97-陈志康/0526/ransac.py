# RANSAC算法步骤：
# 1.一组观察数据（往往含有较大的噪声或无效点）
# 2.一个用于解释观察数据的参数化模型，比如y=ax+b，也可以是曲线
# 3.一些可信的参数

# 实例：
# 1.在数据中随机选择几个点设定为内群
# 2.计算适合内群的模型。如：y=ax+b -> y=2x+3 y=4x+5
# 3.把其他刚才没选到的点带入刚才建立的模型中，计算是否为内群，如：hi=2xi+3->ri
# 4.记下内群数量
# 5.重复以上步骤
# 6.比较哪次计算中内群数量最多，内群最多的那次所建的模型就是我们所要求的的解
