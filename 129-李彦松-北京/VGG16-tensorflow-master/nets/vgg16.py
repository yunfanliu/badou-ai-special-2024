...
def vgg_16(inputs,
           ...
           spatial_squeeze=True, # 最后对输出张量进行平铺
           scope='vgg_16'):
    ...
    with tf.variable_scope(scope, 'vgg_16', [inputs]):
        ...
        if spatial_squeeze:
            net = tf.squeeze(net, [1, 2], name='fc8/squeezed')#tf.squeeze(net, [1, 2])的作用是将卷积层的输出从四维张量(batch_size, height, width, channels)转换为二维张量(batch_size, channels)，这样就可以直接用于分类。
        return net