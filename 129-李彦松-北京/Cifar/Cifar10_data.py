...
def inputs(data_dir,batch_size,distorted):               #这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    ...
    if distorted != None:                         #如果预处理函数中的distorted参数不为空值，就代表要进行图片增强处理
        cropped_image=tf.random_crop(reshaped_image,[24,24,3])          #将预处理好的图片进行剪切，使用tf.random_crop()函数
        ...
        min_queue_examples=int(num_examples_pre_epoch_for_eval * 0.4)  #设置一个最小的队列长度，当队列中的样本数小于这个值时，会添加更多的样本到队列中，以确保在训练过程中有足够的样本可以使用。
