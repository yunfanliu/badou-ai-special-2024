...
'''
optimizer='rmsprop'：优化器决定了网络如何根据训练数据和损失函数来更新权重。'rmsprop'是一种适用于大多数情况的优化器。  
loss='categorical_crossentropy'：损失函数用于评估网络预测的好坏，然后优化器就会使用这个评估结果来更新网络权重。
'categorical_crossentropy'是一种用于多分类问题的损失函数。  
metrics=['accuracy']：在训练和测试过程中，我们想要监控一些指标，比如准确率。这里我们设置了网络训练时要监控的指标为准确率。
'''
...
识别效果与硬件有关（CPU/GPU）.
'''
test_loss, test_acc = network.evaluate(test_images, test_labels, verbose=1) #verbose=1表示输出进度条
print(test_loss) #损失值
print('test_acc', test_acc) #准确率
...
'''
(train_images, train_labels), (test_images, test_labels) = mnist.load_data()
digit = test_images[1] #取第二张图片
plt.imshow(digit, cmap=plt.cm.binary) #cmap=plt.cm.binary表示以黑白灰度显示图片
plt.show()
test_images = test_images.reshape((10000, 28*28))
res = network.predict(test_images)

for i in range(res[1].shape[0]): #在res[1]中找到为1元素，这个元素对应的就是图片的数字
    if (res[1][i] == 1):
