...
from tensorflow_core import truncated_normal, add_to_collection

from session11.cifar10 import Cifar10_data
...
def variable_with_weight_loss(shape, stddev, w1):
    var = tf.Variable(truncated_normal(shape, stddev=stddev))
    if w1 is not None:
        weight_loss = tf.multiply(tf.nn.l2_loss(var), w1, name="weights_loss")
        add_to_collection("losses", weight_loss)
    return var


file_lists = [rf"{data_dir}/data_batch_{i}.bin" for i in range(1, 6)]

images_train, labels_train = Cifar10_data.inputs(file_lists=data_dir, batch_size=batch_size, enhance=True,
                                                 request_num=1000)
images_test, labels_test = Cifar10_data.inputs(file_lists=data_dir, batch_size=batch_size, enhance=None,
                                               request_num=100)
