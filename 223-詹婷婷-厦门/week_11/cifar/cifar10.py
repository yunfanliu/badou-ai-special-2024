...
data_dir = "H:/CV/PRE/pythonProject1/data/cifar-10-batches-bin"

"""
创建一个variable_with_weight_loss()
    1.使用参数w1控制L2 loss的大小
    2.使用函数tf.nn.l2_loss()函数计算权重L2 loss
    3.使用函数tf.multiply()计算权重L2 loss与w1的乘积 并复制给weights_loss
    4.使用函数tf.add_to_collection()将最终的结果放在名为losses的集合里
"""
...
conv1 = tf.nn.conv2d(x, kernel1, [1,1,1,1], padding="SAME")  #tf.nn.conv2d(input, filter, strides, padding, use_cudnn_on_gpu =  Noen, name = None)
