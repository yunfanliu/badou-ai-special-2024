...
def vgg_16(inputs, num_classes=1000, is_training=True, dropout_keep_prob=0.5, spatial_squeeze=True, scope='vgg_16'):
    with tf.variable_scope(scope, 'vgg_16', [inputs]):  # 建立vgg_16的网络结构
        ...
        # 利用卷积的方式模拟全连接层，而不用flatten，效果等同，输出net为(1,1,4096)
        ...
        # 由于用卷积的方式模拟全连接层，所以输出需要平铺，等效于flatten
        if spatial_squeeze:
            net = tf.squeeze(net, [1, 2], name='fc8/squeezed')  # 将net在第1,2个维度上的大小为1的维度去除，通常用于从卷积层到全连接层的转换
        return net
