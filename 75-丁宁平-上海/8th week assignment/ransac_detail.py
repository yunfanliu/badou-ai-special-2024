...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    ...
    输入
        ...
        :param k: 最大迭代次数
        :param t: 阈值：作为判断点满足模型的田间
        :param d: 拟合较好时，需要的样本点最少的个数，当作阈值看待
    输出
        :return: 最优拟合解 (返回nil,如果没找到)
...
def func():
    ...
    n_inputs = 1   # 输入变量的个数
    n_outputs = 1  # 输出变量的个数
    A_exact = 20 * np.random.random((n_sample, n_inputs))  # 随机生成0-20之间的500个数据，(500,1)(行，列)
    perfect_fit = 60 * np.random.normal(size=(n_inputs, n_outputs))  # 随机线性度，即机生成一个斜率
