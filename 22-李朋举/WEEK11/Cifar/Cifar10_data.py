...
def read_cifar10(file_queue):
    ...
    label_bytes = 1  # 10个分类结果, 用数字0-9来表示, labels[x],x属于0-9  （如果是Cifar-100数据集，则此处为2）
    result.height = 32
    result.width = 32
    ...
    # 这一步是将一维数据转换成3维数据 (c,h,w)
    depth_major = tf.reshape(tf.strided_slice(record_bytes, [label_bytes], [label_bytes + image_bytes]),
                             [result.depth, result.height, result.width])

    # 我们要将之前分割好的图片数据使用tf.transpose()函数转换成为高度信息、宽度信息、深度信息这样的顺序
    # 这一步是转换数据排布方式，变为(h,w,c) (tf两种格式都可以接受)
    # reshape 改变维度   transpose 改变维度的顺序
...
def inputs(data_dir, batch_size, distorted):  # 这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    ...
    if distorted != None:  # 如果预处理函数中的distorted参数不为空值，就代表要进行图片增强处理 （数量、质量）
